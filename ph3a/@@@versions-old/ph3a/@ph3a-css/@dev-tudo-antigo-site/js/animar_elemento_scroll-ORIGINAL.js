	...
	$(window).on(
		...
		function() {
			...
			if (new Date().getTime() - millis_ultima_atualizacao_cache_data_animar_scroll > 2000) {
				...
				//Não se pode utilizar -->>$(window).height()<<-- pois em celulares, quando se rola para baixo, a barra de ferramentas de busca é ocultada pelo navegador e -->>$(window).height()<<-- acaba não retornando o valor corrigido da altura da tela considerando a ocultação da barra de ferramentas. Em dispositivos que não sejam móveis não se deve utilizar o código abaixo (e sim o "else" mais abaixo pois se pressionar F12 e a tela de desenvolvedor estiver presa na parte inferior da tela, então a altura da tela retornada pelo código abaixo não considerará o tamanho agora reduzido da tela útil do site.
			...
			elemento_data_animar_scroll.not(".data-animar-scroll-esta-animando").each(
				...
				function() {
					...
					//Não junte os 2 "if" abaixo pois da forma que estáa baixo, o código será executado de forma mais otimizada.
		...
		function() {
			...
			if (new Date().getTime() - millis_ultima_atualizacao_cache_data_animar_scroll > 2000) {
				...
				elemento_data_animar_scroll.each(
					...
					function() {
						...
						//É importante utilizar "data-animar-scroll-ultimo-inicio-animacao" pois algumas animações, como "fadeInUp", alteram o posicionamento vertical do elemento (retornado pela função "offset") durante a animação e não há nenhum outro recurso/função/variável confiável que se possa utiizar para capturar o posicionamento vertical do elemento desconsiderando as alterações no posicionamento do elemento realizadas pela animação. Algumas animações (como "transform") apesar de não alterarem diretamente o atributo CSS "top", acabam alterando o posicionamento vertical retornado por "offset". O único inconveniente do "hack" abaixo é que se alguma animação demorar mais do que 3000 ms para finalizar, o sistema poderá capturar o posicionamento vertical do elemento enquanto ele ainda estava sendo animado e isso poderá fazer com que o momento de reaparecimento do elemento na tela não seja conforme esperado. Não se aconselha utilizar "transitionend MSTransitionEnd webkitTransitionEnd oTransitionEnd animationend webkitAnimationEnd" para detectar quando uma animação foi finalizada pois, nos teste que se realizou, isso não é confiável.
			...
			elemento_data_animar_scroll.each(
				...
				function() {
					...
					//Não junte os 2 "if" abaixo pois da forma que está abaixo, o código será executado de forma mais otimizada.
					if ($(this).is(".data-animar-scroll-ocultou-elemento")) {
